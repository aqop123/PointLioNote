 ...
 * 处理切割LiDAR数据帧的函数
 * 
 * 该函数用于处理来自Livox LiDAR的点云数据，根据特定的条件筛选有效点，并按照时间顺序进行排序和切割
 * 
 * @param msg LiDAR数据的消息指针，包含点云数据和元数据
 * @param pcl_out 一个双端队列，用于存储处理后的点云数据
 * @param time_lidar 一个双端队列，用于存储每个帧的时间戳
 * @param required_frame_num 需要的帧数，用于确定如何切割点云
 * @param scan_count 当前扫描计数，用于调整切割策略
 */
void Preprocess::process_cut_frame_livox(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg,
